...
//WALKSPD, FILEPATH, MAXSKIP, DPDIMENSION , unitConv FROM GlobCostPXIBD5
	...
	// check to see if this is a scene data and if so find if the list is from a DP result database 
	...
	if (blnScene) {
		...
		// try opening it as a sqlite table
			...
			strDPTblName = rec1 + "%DPTrace%"; //rec1 + "%" + strWR + "%DPTrace%";
...
// read the original edge data from the edge file
		...
		maped0.clear();
		readBinEdgeObjectData(maped0,ev,infile,onoff);
...
// read the original edge data from the edge file
		...
		infile.open(strInFname.c_str(), ios::in | ios::binary);
		maped1.clear();
		readBinEdgeObjectData(maped1,ev,infile,onoff);
